...
[
    'name' => 'product.offer.delete',
          ...
          // صلاحيات سندات إخراج الكميات
          ['name' => 'stock_out.create'], // إضافة سند إخراج
          ['name' => 'stock_out.delete'], // حذف سند إخراج
          ['name' => 'stock_out.view'],   // عرض السندات (للمراجعة)
        ...
        foreach ($data as $d) {
            // نجهز البيانات لكل صلاحية
            $d['guard_name'] = 'web';
            $d['created_at'] = $time_stamp;

            // نستخدم firstOrCreate داخل الحلقة للتأكد من كل اسم على حدة
            // نبحث بالاسم فقط، وإذا لم تكن موجودة ننشئها ببقية البيانات
            Permission::firstOrCreate(['name' => $d['name']], $d);
        }
