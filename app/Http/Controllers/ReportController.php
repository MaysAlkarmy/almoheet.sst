    ...
    $full_end = \Carbon\Carbon::now()->endOfDay()->toDateTimeString();
         ...
         $due_res = $due_q->selectRaw("
          ...
          ")->first();          
           ...
           // --- حساب إجمالي المبيعات (أي عملية نوعها sell) ---
$total_sell_q = \App\Transaction::where('business_id', $business_id)
    ->where('type', 'sell')
    ->where('status', 'final');

if ($location_id) $total_sell_q->where('location_id', $location_id);
if ($full_start && $full_end) {
    $total_sell_q->whereBetween('transaction_date', [$full_start, $full_end]);
}

// نجلب المجموع الإجمالي للفواتير وعددها
$total_sell_res = $total_sell_q->selectRaw("SUM(final_total) as grand_total, COUNT(id) as total_count")->first();

$grand_total_combined = (float)($total_sell_res->grand_total ?? 0);
$total_transactions_count = (int)($total_sell_res->total_count ?? 0); 
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->select(
                    ...
                    'v.default_purchase_price as purchase_price',  /// purchase price 001
            ...
            return Datatables::of($query)
                ...
                // هنا نقوم بضرب سعر الشراء في الكمية للحصول على إجمالي تكلفة السطر
                // إذا كنت تريد مجموع أسعار الوحدات فقط، احذف الضرب في الكمية
                 $total_purchase = $row->purchase_price * $row->sell_qty;

                return '<span class="purchase_price" data-orig-value="' . $row->purchase_price . '">'.
...
if (!empty($full_start) && !empty($full_end)) {
  ...
  //////////////////  تعديل ل ادخال كميات عشان تتسجل بسجل النشاطات  001
    ...
    ////////// 001 تقرير المبيعات اليومي المجمل
 ...
 ////////// 001 تقرير المبيعات اليومي التفصيلي
